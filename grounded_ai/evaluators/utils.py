...
from grounded_ai.validators.output_data import OutputInstance
...
def debug_eval_logging(func):
    ...
    def wrapper(evaluator, data, *args, **kwargs):
        ...
        logger.debug(f"Input data sample: {str(data[:2]) if len(data) > 2 else str(data)}")
...
def log_instance(instance_num: int, instance_data: dict, output: str, classification: str):
    """
    ...
    if 'context' in instance_data and 'query' in instance_data:  # RAG
        logger.debug(f"Instance {instance_num}: Context='{str(instance_data.get('context', ''))[:100]}...', Query='{instance_data.get('query', '')}'")
    elif 'text' in instance_data:  # Toxicity
        logger.debug(f"Instance {instance_num}: Text='{str(instance_data.get('text', ''))[:100]}...'")
    elif 'query' in instance_data and 'response' in instance_data:  # Hallucination
        logger.debug(f"Instance {instance_num}: Query='{str(instance_data.get('query', ''))[:50]}...', Response='{str(instance_data.get('response', ''))[:50]}...'")
    
    # Log output and classification
    logger.debug(f"Instance {instance_num} - Raw output: '{output}', Classification: '{classification}'")
...
def evaluate_rag(evaluator, data: List[Tuple[str, str]]) -> dict:
    ...
    relevant = 0
    unrelated = 0
    for i, instance in enumerate(tqdm(evaluation_data.instances, desc="Evaluating RAG Relevance")):
...
def evaluate_toxicity(evaluator, data: List[str]) -> dict:
    ...
    for i, instance in enumerate(tqdm(evaluation_data.instances, desc="Evaluating Toxicity")):
...
def evaluate_hallucination(
    ...
    try:
        ...
        hallucinated: int = 0
        truthful: int = 0
        for i, instance in enumerate(tqdm(evaluation_data.instances, desc="Evaluating Hallucination")):
            instance = instance.model_dump()
            output = evaluator.run_model(instance)
            output_instance = OutputInstance(raw_response=output)
            classification = output_instance.rating
